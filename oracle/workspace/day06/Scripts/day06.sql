...
SELECT E.JOB_ID, J.JOB_TITLE, E.EMAIL, E.LAST_NAME, E.FIRST_NAME
...
-- 조인후 한 테이블 전체 내용 가지고 오고 싶을 땐 알파벳.*을 입력
SELECT E.*, DEPARTMENT_NAME
FROM DEPARTMENTS D JOIN EMPLOYEES E
ON D.DEPARTMENT_ID = E.DEPARTMENT_ID AND HIRE_DATE
BETWEEN '2003-01-01' AND '2005-12-31';

--TO_DATE를 사용할 떄에 년도만 설정하면, 월은 SYSDATE의 월, 일은 01로 자동 설정된다.
SELECT E.*, DEPARTMENT_NAME
FROM DEPARTMENTS D JOIN EMPLOYEES E
ON D.DEPARTMENT_ID = E.DEPARTMENT_ID AND HIRE_DATE
BETWEEN TO_DATE('20030101', 'YYYYMMDD') AND TO_DATE('20051231', 'YYYYMMDD'); 
...
-- STEP 1) 우선 팀별로 가장 큰 키를 검색해 묶는다
-- STEP 2) 플레이어 테이블과 조인한다.
-- STEP 3) P1 최대 키와 P2에 있는 HEIGHT를 비교하여 일치하면 대입한다.
SELECT *
FROM
(
